...
void cargar_pcb_paginacion(t_list*, t_list*, t_list*, uint32_t, uint32_t);
t_tabla_de_paginas *encontrar_pid_lista_tablas(t_list*, uint32_t);
void cargar_tareas_paginacion(t_list*, t_list*,t_list*, uint32_t, uint32_t, char*);
void cargar_tripulante_paginacion(uint32_t, t_tcb*);
...
bool hay_espacio_disponible(t_list*, t_list*, uint32_t, uint32_t, uint32_t);
void dump_memoria();
